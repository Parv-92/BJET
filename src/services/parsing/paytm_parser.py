...
class PaytmParser(BaseReceiptParser):
    """Specialized parser for Paytm UPI payment receipts."""

    APP_NAME = "Paytm"
    IDENTIFIERS = ["paytm", "paid successfully to", "upi ref no", "order id"]

    MERCHANT_PATTERNS = [
        re.compile(r"Paid successfully to\s*\n*([A-Za-z0-9\s&.\-']+?)(?:\n|$|\+91|\d{10}|[a-zA-Z0-9.\-_]+@)", re.IGNORECASE),
        re.compile(r"Paid to\s*\n*([A-Za-z0-9\s&.\-']+?)(?:\n|$|\+91|\d{10}|[a-zA-Z0-9.\-_]+@)", re.IGNORECASE),
        re.compile(r"To\s*\n*([A-Za-z0-9\s&.\-']+?)(?:\n|$|\+91|\d{10}|[a-zA-Z0-9.\-_]+@)", re.IGNORECASE),
    ]

    PAYTM_UPI_REF_PATTERN = re.compile(r"UPI\s*Ref\s*(?:No\.?|ID)?\s*[:\-]?\s*([0-9]{12})", re.IGNORECASE)
    ...
    def parse(self, text: str) -> ExtractedTransaction:
        ...
        upi_ref = self.extract_paytm_ref(text) or self.extract_upi_ref(text)
    ...
    def extract_paytm_ref(self, text: str) -> Optional[str]:
        match = self.PAYTM_UPI_REF_PATTERN.search(text)
