...
class GooglePayParser(BaseReceiptParser):
    """Specialized parser for Google Pay UPI payment receipts."""

    APP_NAME = "Google Pay"
    IDENTIFIERS = ["google pay", "gpay", "google transaction id", "upi transaction id"]

    MERCHANT_PATTERNS = [
        re.compile(r"Paid to\s+([A-Za-z0-9\s&.\-']+?)(?:\n|$|\+91|\d{10}|[a-zA-Z0-9.\-_]+@)", re.IGNORECASE),
        re.compile(r"To\s+([A-Za-z0-9\s&.\-']+?)(?:\n|$|\+91|\d{10}|[a-zA-Z0-9.\-_]+@)", re.IGNORECASE),
        re.compile(r"Paying\s+([A-Za-z0-9\s&.\-']+?)(?:\n|$|\+91|\d{10}|[a-zA-Z0-9.\-_]+@)", re.IGNORECASE),
    ]
